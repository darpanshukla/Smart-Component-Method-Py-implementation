...
def construct_test_tr(component):
	...
	transition_str += 'cyclic:' + '%s:'%(float(temp[1][component]['TestTime'])/3600.0)  + '0 '#'%d '%(temp[1][component]['TestOffset'])
		...
		if component in listofCCFG['representative']:
			...
			for i in range(ccf):
				...
				if listofCCFG['CCFG%d'%ccfid]['Repre'] == component:
					...
					l2    = beta1 * l1
					beta += beta1
					listoftransition.append( construct_CC_failure_tr( component, l2, ccfid ) )
			l = (1.0 - beta) * l1
		else:
...
def generate_hl_series_traditional_model( listofcid, lsplaces, placename, hl, id ):
	pflist = []
	for i in listofcid:
		# gatherig indicator places
		pflist.append( lsplaces[i] )
	transitions = generate_hl_series_transitions( pflist, placename, hl, id )
	return transitions;

def make_place_transition_series_connections(se_connection, lsplaces, hl):
	transitionlist = []
	for i in range(len(se_connection)):
		n = len(se_connection[i])
		if n > 1:
			placename = 'HLS%dID%d'%( hl, i )
			lsp[hl].append(placename)
			transition = generate_hl_series_traditional_model( se_connection[i], lsplaces, placename, hl, i ) 
			lsplaces[se_connection[i][0]] = placename
			for j in transition:
...
def general_k_n_place_transition( k, se_connection, slist, lsplaces, hl, pid ):
	placelist      = []
	transitionlist = []
	gcplace        = 'GCount%dI%d'%( hl, pid )
	lsp[hl].append( gcplace )
	for i in range( len( slist ) ):
		## identify the input place 
		ip      = lsplaces[ se_connection[ slist[i] ][0] ]	
		## semaphore place
		spplace = 'SP%dI%di%d'%( hl, pid, i   )
		lsp[hl].append( spplace )
...
def make_place_transition_parallel_connections( se_connection, para_connection, lsplaces, hl, compnamelist):
	lstransitions = []
	for i in range(len(para_connection)):
		...
		[ lsplaces, transitions ] = general_k_n_place_transition( k, se_connection, para_connection[i][0], lsplaces, hl, i )
		
		## Place for the parallel collapse
		PCplace = 'PCc%dI%d'%( hl, i )
		if hl == 1:
			PCcompplace = temp[ 1 ][ compnamelist[ para_connection[i][1] ]]['Places'][-1]
			# trn = two_place_series(PCcompplace, lsp[hl][-1], PCplace)
		else:
			PCcompplace = lsplaces[ para_connection[i][1]]
		trn = two_place_series(PCcompplace, lsp[hl][-1], PCplace)
			#lsp[hl].append( PCcompplace )
		lsp[hl].append( PCplace     )
		lsplaces[ para_connection[i][1] ] = PCplace
...
def identify_places_transition( se_connection, para_connection, lsplaces, hl, compnamelist ):
	...
	[ lsplaces, stransitions ] = make_place_transition_series_connections(   se_connection,                  lsplaces, hl) 
	[ lsplaces, ptransitions ] = make_place_transition_parallel_connections( se_connection, para_connection, lsplaces, hl, compnamelist) 
...
if __name__ == "__main__":
	...
	for i in temp[0]:
		...
		listofplaces.append(places_list[-1])
		for j in range(len(places_list)):
			lsp[0].append(places_list[j])
