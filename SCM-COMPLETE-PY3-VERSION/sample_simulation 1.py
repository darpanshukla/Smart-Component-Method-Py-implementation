...
def sample_state( weight2, tt, batch, comp = 0 ):
	...
	if im.firsttime in [ 1, 2 ]:#Simple Failure Biasing
		...
		for i in range(im.lenc):
			if ( (zita2 >= ( float( i ) / float( im.lenc )))  and (zita2<(float(i + 1)/float(im.lenc)))):
				if im.state[i] == 1:
					failcomp(i)
					weight2 = weight2 * im.lenc * float(im.rates[1][i]) / float(g2)		
				
				elif ((im.Rel_Model[i] in [5,6]) and (im.state[i] in [2])) or (im.Rel_Model[i]==3 and (im.state[i] in [0])):
					repaircomp(i)
					weight2 = weight2 * im.lenc * float(im.rates[0][i]) / float(g2)	
				
				elif ((im.Rel_Model[i] in [5,6]) and (im.state[i] in [0])) :
					im.state[i] = 2
					weight2 = weight2 * im.lenc * float(im.TestRate[i]) / float(g2)	
				break
		return weight2
