...
def opwrite():
    ...
    L                  = [eval(str(im.compsdata[im.comps[i]]['Failure Rate']))for i in range(len(im.comps))]
    ...
    file.write('\n{:>4}\t{:>20}\tRel.Model(Law)\t{:>20}\trepair_rate\t test_interval\t MTTR\t CCF\t CCF_ID\t Beta'.format(temp[0],temp[1], temp[3]))

    for i1 in range(len(im.comps)):
        file.write('\n{}\t{:<20}\t{:>5} ({:2})\t{:20}\t{}\t{:.2e}\t{:.2e}\t{}\t{}'.format( i1, im.comps[i1], im.Rel_Model[i1], int(im.fetchdata(im.comps[i1],"Law")), str(L[i1]), mu[i1], im.TestingInterval[i1], im.MTTR[i1], int(im.fetchdata(im.comps[i1],'CCF')), str(im.ccf_id[i1]) ))
